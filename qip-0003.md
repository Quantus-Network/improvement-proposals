...
NOTE: This proposal has been superseded by QIP-0013
...
def poseidon2_512(input):
    # Historical sketch only: chained double-hash.
    # Production PoW (QIP-0013) uses one Poseidon2 absorb with two squeezes.
...
This RSA-shortcut design is **no longer present in the runtime**. The current Poseidon2 PoW is specified in [QIP-0013](./qip-0013.md) and implemented in [qpow-math](https://github.com/Quantus-Network/chain/tree/main/qpow-math) / [pallets/qpow](https://github.com/Quantus-Network/chain/tree/main/pallets/qpow).

Poseidon2 lives [here](https://github.com/Quantus-Network/qp-poseidon). See [QIP-0013](./qip-0013.md) for the parameters and hashing construction actually deployed (one absorb, two squeezes — not the chained double-hash above).
...
We changed to poseidon2 from SHA2 and SHA3 because we want block header validation to be easy to prove in a zk-circuit and SHA is notoriously inefficient for this. The historical RSA-shortcut sketch above assumed a different sponge layout and hashing construction than production PoW. Deployed Poseidon2 parameters are documented in [QIP-0013](./qip-0013.md) (`width = 12`, `rate = 8`, `capacity = 4` ⇒ ≈128 bits classical capacity security under $2^{c/2}$). 
