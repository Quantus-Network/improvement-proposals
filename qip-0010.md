...
We use a [modified scheduler](qip-0009.md) that uses timestamps for scheduling instead of block-number, for better user experience. Clients schedule delayed transfers via:

- [`schedule_transfer`](https://github.com/Quantus-Network/chain/blob/main/pallets/reversible-transfers/src/lib.rs) — **high-security accounts only**; uses the account's configured delay (`AccountNotHighSecurity` otherwise)
- [`schedule_transfer_with_delay`](https://github.com/Quantus-Network/chain/blob/main/pallets/reversible-transfers/src/lib.rs) — ordinary accounts only; caller-chosen delay (`AccountAlreadyReversibleCannotScheduleOneTime` once an account is high-security)

Funds are placed on hold until the scheduled dispatch completes or the transfer is cancelled.

To cancel a pending transfer, call [`cancel`](https://github.com/Quantus-Network/chain/blob/main/pallets/reversible-transfers/src/lib.rs) with the pending transfer id. For ordinary (non-high-security) transfers the sender may cancel and receive a full refund. High-security cancel semantics are defined in [QIP-0011](qip-0011.md).
...
Calling into the scheduler is fairly complex, so we wrap it in a simple extrinsic interface and hold funds until completion or cancellation. Splitting scheduling into high-security-only vs one-time-delay extrinsics keeps mandatory delays enforceable for guarded accounts without blocking casual delayed sends.
