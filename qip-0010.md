...
- [`schedule_transfer`](https://github.com/Quantus-Network/chain/blob/main/pallets/reversible-transfers/src/lib.rs) — uses the account's configured delay (required for high-security accounts)
- [`schedule_transfer_with_delay`](https://github.com/Quantus-Network/chain/blob/main/pallets/reversible-transfers/src/lib.rs) — caller-chosen delay (not available once an account is high-security)
...
Calling into the scheduler is fairly complex, so we wrap it in a simple extrinsic interface and hold funds until completion or cancellation.
