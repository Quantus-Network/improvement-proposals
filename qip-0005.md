...
(nullifiers, block_hash, parent_hash, block_number, exit_addresses, exit_amounts) then become the limiting factor, with ~256 additional bytes per transaction. 
...
to the address generates a [TransferProof](https://github.com/Quantus-Network/chain/blob/main/pallets/balances/src/lib.rs#L524) storage object. The user's wallet can generate a storage proof for each 
...
It's important to not that we only support aggregation of transfer proofs across contigious blocks. The reason being is that the circuit verifies connectivity of deduplicated blocks via parent-hash and doesn't account for gaps for the purposes of keeping the circuit constaints low. New sets of aggregated proofs must be created for transfer proofs from non-contiguous blocks.
...
- That the block header (parent hash, block number, storage root, extrinsic root and digest) hashes to the supplied block hash.
