...
Address poisoning attacks can be mitigated by using a cryptographically-secure human-readable checksum. This checksum is generated using a cryptographic hash function and is represented as a sequence of words that are easy for humans to remember and verify, using the same principle as the [BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki) mnemonic phrase.

## Specification

We use PBKDF2-HMAC-SHA256 with 40,000 iterations and the salt `"human-readable-checksum"` on the address bytes (no normalization beyond what the caller supplies) to derive `KEY_BYTECOUNT = ceil(5 * 11 / 8) = 7` bytes.

Those 7 bytes are interpreted as a big-endian integer; the implementation then keeps the **top 55 bits** via
`key_int >>= (8 * KEY_BYTECOUNT) % 11` (i.e. `>>= 1`), splits them into five 11-bit indices, and maps each index into the curated word list. An implementation that instead takes the *low* 55 bits will produce a different checkphrase for the same address and must be considered non-interoperable.

The word list is the exact 2048-line artifact [`final_wordlist.txt`](https://github.com/Quantus-Network/qp-human-checkphrase/blob/main/final_wordlist.txt) in the reference repository (BIP39-inspired and curated for positive/recognizable words; it is **not** identical to BIP39 English — only a subset of words is shared). Prefixes are *mostly* unique at 4 characters (28 collision groups remain in the shipped list — do **not** rely on 4-char abbreviation or autocomplete uniqueness). Wallets must use this file byte-for-byte.

This check-phrase is shown to the user wherever the address is displayed.

## Reference Implementation

The check-phrase is implemented [here](https://github.com/Quantus-Network/qp-human-checkphrase) (Rust, with JS and Dart bindings).

## Rationale

We use PBKDF2-HMAC-SHA256 because it requires sequential hashing of the input, which makes it more resistant to brute-force attacks. The number of iterations (40k) is chosen to make it quick to generate dynamically in UIs (~14ms on typical CPUs), while still requiring a very long time to generate a rainbow-table. The salt makes it even less likely that an attacker would have already generated a rainbow-table that contains the hash of the address. The number of words is chosen to be 5 to ensure a high level of security while still being easy to read, say and remember. Interoperability requires identical KDF parameters, bit-slicing, and word-list bytes across wallets — otherwise anti-poisoning comparisons fail silently.
