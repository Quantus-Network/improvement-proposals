...
Address poisoning attacks can be mitigated by using a cryptographically-secure human-readable checksum. This checksum is generated using a cryptographic hash function and is represented as a sequence of words that are easy for humans to remember and verify, using the same principle (and a 2048-word list in the BIP39 style) as the [BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki) mnemonic phrase.

## Specification

We use PBKDF2-HMAC-SHA256 with 40,000 iterations and the salt `"human-readable-checksum"` to derive 7 key bytes from the address string (`ceil(5 * 11 / 8)`). Those bits are split into five 11-bit indices and mapped to 5 words from a curated 2048-word list (BIP39-based, filtered toward positive/recognizable words with unique 4-character prefixes).
